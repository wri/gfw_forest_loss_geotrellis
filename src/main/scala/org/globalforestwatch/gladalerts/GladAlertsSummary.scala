  ...
  implicit val mdhCellRegisterForTreeLossRaster1
    ...
    new CellVisitor[Raster[GladAlertsTile], GladAlertsSummary] {
      ...
      def register(raster: Raster[GladAlertsTile],
        ...
        val glad: Option[(LocalDate, Boolean)] = raster.tile.glad.getData(col, row)
        ...
        def updateSummary(
          ...
          if (tile.z < 0) stats
            ...
            val alertDate: Option[LocalDate] = {
              glad match {
                case Some(g: (LocalDate, Boolean)) => Some(g._1)
                case _ => null
              }
            }
            ...
            val summary: GladAlertsData =
              stats.getOrElse(key = pKey, default = GladAlertsData(0, 0, 0, 0))
