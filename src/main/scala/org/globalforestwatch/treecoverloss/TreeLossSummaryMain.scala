...
object TreeLossSummaryMain extends CommandApp (
  ...
  main = {
    ...
    (
      ...
      val WindowPartitionOrder = Window.partitionBy($"m_feature_id", $"m_layers").orderBy($"m_threshold".desc)
      ...
      val tAdm2DF = summaryDF
        ...
        .select($"feature_id" as "m_feature_id", $"layers" as "m_layers", $"threshold" as "m_threshold", $"totalarea")
      ...
      val sAnnualLossDF = summaryDF
        .select(
          ...
          $"area", $"biomass", $"co2", $"mangrove_biomass", $"mangrove_co2",
      ...
      val tAnnualLossDF = sAnnualLossDF
        .join(tAdm2DF, $"feature_id" <=> $"m_feature_id"
          && $"layers" <=> $"m_layers" && $"threshold" <=> $"m_threshold"
          , "right_outer")
        .na.fill(0.0, Seq("area", "biomass", "co2", "mangrove_biomass", "mangrove_co2",
        "area_loss_2001",
        "area_loss_2002",
        "area_loss_2003",
        "area_loss_2004",
        ...
        .na.fill(2001, Seq("year_2001"))
        .na.fill(2002, Seq("year_2002"))
        .na.fill(2003, Seq("year_2003"))
        .na.fill(2004, Seq("year_2004"))
        .na.fill(2005, Seq("year_2005"))
        .na.fill(2006, Seq("year_2006"))
        .na.fill(2007, Seq("year_2007"))
        .na.fill(2008, Seq("year_2008"))
        .na.fill(2009, Seq("year_2009"))
        .na.fill(2010, Seq("year_2010"))
        .na.fill(2011, Seq("year_2011"))
        .na.fill(2012, Seq("year_2012"))
        .na.fill(2013, Seq("year_2013"))
        .na.fill(2014, Seq("year_2014"))
        .na.fill(2015, Seq("year_2015"))
        .na.fill(2016, Seq("year_2016"))
        .na.fill(2017, Seq("year_2017"))
        .na.fill(2018, Seq("year_2018"))
        .select($"m_feature_id" as "feature_id", $"m_threshold" as "threshold", $"m_layers" as "layers", $"totalarea",
      ...
      val s2010DF = summaryDF
        ...
        .join(tAdm2DF, $"m_feature_id" <=> $"feature_id" &&
          $"m_layers" <=> $"layers" &&
          $"m_threshold" <=> $"threshold"
          , "right_outer")
        .na.fill(0.0, Seq("area"))
        .select($"m_feature_id", $"m_layers", $"m_threshold", windowSum("area") as "extent2010")


      val adm2DF = tAnnualLossDF.
        join(s2010DF, $"m_feature_id" <=> $"feature_id" &&
          $"m_layers" <=> $"layers" &&
          $"m_threshold" <=> $"threshold"
          , "left_outer")
        .select($"feature_id.iso" as "iso",
