      ...
      def visit(raster: Raster[AFiTile], col: Int, row: Int): Unit = {
        ...
        if (naturalForestCategory == "Natural Forest") {
          ...
          if (lossYear >= AFiCommand.TreeCoverLossYearStart) {
            ...
            summaryData.natural_forest_loss_by_year__ha = summaryData.natural_forest_loss_by_year__ha.merge(ForestChangeDiagnosticDataLossYearly.fill(lossYear, areaHa, include = true, minLossYear = AFiCommand.TreeCoverLossYearStart,
        ...
        if (jrcForestCover) {
          ...
          if (lossYear >= AFiCommand.TreeCoverLossYearStart) {
            ...
            summaryData.jrc_forest_loss_by_year__ha = summaryData.jrc_forest_loss_by_year__ha.merge(ForestChangeDiagnosticDataLossYearly.fill(lossYear, areaHa, include = true, minLossYear = AFiCommand.TreeCoverLossYearStart,
            maxLossYear = AFiCommand.TreeCoverLossYearEnd))
          }
        }
