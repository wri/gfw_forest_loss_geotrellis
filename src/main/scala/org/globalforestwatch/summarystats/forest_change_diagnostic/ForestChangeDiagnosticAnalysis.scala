...
object ForestChangeDiagnosticAnalysis extends SummaryAnalysis {
                ...
                .mapValues { case (data, fire) =>
                  if (data.equals(ForestChangeDiagnosticData.empty)) {
                    ...
                    data.copy(
                      ...
                      tree_cover_loss_soy_yearly = data.tree_cover_loss_soy_yearly.limitToMaxYear(2024)
                    )
                  }
                }
            }
  ...
  def fireStats(
    ...
    // This fire data is an input to the palm risk tool, so limit data to 2024 to sync
    ...
    joinedRDD.rdd
      ...
      .mapValues { fires =>
        aggregateFireData(fires.merge(ForestChangeDiagnosticDataLossYearly.prefilled)).limitToMaxYear(2024)
      }
  }
