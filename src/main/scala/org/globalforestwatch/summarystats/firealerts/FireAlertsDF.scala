...
object FireAlertsDF {
  ...
  def aggChangeDaily(groupByCols: List[String],
    ...
    val cols =
      if (!wdpa)
        groupByCols ::: fireCols ::: "wdpa_protected_areas__iucn_cat" :: "wdpa_protected_area__iucn_cat" :: contextualLayers
      else
  ...
  private def _aggChangeWeekly(df: DataFrame,
    ...
    val contextLayers: List[String] =
      if (!wdpa) "wdpa_protected_areas__iucn_cat" :: "wdpa_protected_area__iucn_cat" :: contextualLayers
