...
object CarbonFluxMinimalRDD extends SummaryRDD {

  type SOURCES = CarbonFluxMinimalGridSources
  type SUMMARY = CarbonFluxMinimalSummary
  type TILE = CarbonFluxMinimalTile

  def getSources(windowKey: SpatialKey, windowLayout: LayoutDefinition, kwargs: Map[String, Any]): Either[Throwable, SOURCES] = {
    Either.catchNonFatal {
      CarbonFluxMinimalGrid.getRasterSource(windowKey, windowLayout, kwargs)
    }
  }
  ...
  def runPolygonalSummary(raster: Raster[TILE],
    ...
    raster.polygonalSummary(
      geometry,
      CarbonFluxMinimalSummary.getGridVisitor(kwargs),
      options = options
    )
  }
